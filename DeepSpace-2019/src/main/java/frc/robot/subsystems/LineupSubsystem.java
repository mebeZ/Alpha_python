...
public class LineupSubsystem extends Subsystem {
    ...
    public void resetInfo(double forwardChange, double shiftChange, double angleChange) {
    	...
    	Robot.tankDriveSubsystem.resetTurnPID(0.5,.15,0.3);
    	}
    ...
    public void move(){
    	if (parallelCoordinate() < desiredForward) {
	        ...
	        Robot.tankDriveSubsystem.setRightSpeed(defaultSpeed * (1 + output)); // The output changes the percentage that goes to each side which makes it turn
            Robot.tankDriveSubsystem.setLeftSpeed(defaultSpeed * (1 - output));
        }
    	else {
            Robot.tankDriveSubsystem.setRightSpeed(0.0);
            Robot.tankDriveSubsystem.setLeftSpeed(0.0);
    	}
    }
